...
const acceptRide = async (rideId: string, driverId: string) => {
  ...
  if (ride.driver)
    throw new AppError(StatusCodes.BAD_REQUEST, "Ride already accepted");
  if (ride.status !== RideStatus.REQUESTED)
    throw new AppError(StatusCodes.BAD_REQUEST, "Ride cannot be accepted");

  const driver = await User.findById(driverId);
  if (!driver?.isActive || !driver?.isApproved)
    throw new AppError(
      StatusCodes.FORBIDDEN,
      "You are not allowed to accept rides"
    );
...
const getSingleRide = async (driverId: string) => {
  if (!driverId)
    throw new AppError(StatusCodes.BAD_REQUEST, "Driver ID is required");
...
const getMyRides = async (riderId: string) => {
  return await Ride.find({ rider: new Types.ObjectId(riderId) })
    .populate("driver")
    .populate("rider")
    .sort({ createdAt: -1 });
};
...
const getDriverRides = async (driverId: string) => {
  return await Ride.find({ driver: new Types.ObjectId(driverId) })
    .populate("rider")
    .sort({ createdAt: -1 });
};
...
const rateDriver = async (
  ...
  const totalScore =
    (driver.averageRating || 0) * (driver.totalRatings || 0) + score;
